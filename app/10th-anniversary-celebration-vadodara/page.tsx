...
const celebrationPackages = [
  ...
  {
    name: "Perfect 10 Party",
    price: "₹34,999",
    duration: "5 hours",
    guests: "Up to 30 guests",
    highlights: [
      "Decorated celebration venue",
      "10-year timeline display",
      "Cocktail + dinner",
  ...
  },
  {
    ...
    highlights: [
      "Premium venue setup",
      "Documentary-style timeline",
      "Full catering service",
      "Photo + video coverage",
      "Live entertainment",
      "Guest memory station",
      "Surprise elements",
      ...
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4">
          ...
          <p className="text-gray-600 text-center mb-12 max-w-2xl mx-auto">
            Celebrate a decade of love in the style that suits you
          </p>
            ...
            {celebrationPackages.map((pkg, index) => (
              <div
                ...
                className={`rounded-2xl p-8 ${
                  pkg.popular
                    ? 'bg-gradient-to-br from-amber-500 to-orange-500 text-white ring-2 ring-amber-300'
              ...
              >
                ...
                <div className={`flex items-center gap-4 mb-4 text-sm ${pkg.popular ? 'text-amber-100' : 'text-amber-600'}`}>
                ...
                <div className={`text-4xl font-bold mb-6 ${pkg.popular ? 'text-white' : 'text-amber-600'}`}>
                  {pkg.price}
                </div>
                <ul className="space-y-3 mb-8">
                  {pkg.highlights.map((item, i) => (
                    <li key={i} className="flex items-start gap-2">
                      <Star className={`w-4 h-4 mt-1 flex-shrink-0 ${pkg.popular ? 'text-amber-200' : 'text-amber-500'}`} />
                      <span className={pkg.popular ? 'text-amber-50' : 'text-amber-700'}>{item}</span>
                    </li>
                  ))}
                </ul>
                <a
                  href={`https://wa.me/917487888730?text=Hi! I'm interested in the ${pkg.name} package for our 10th anniversary`}

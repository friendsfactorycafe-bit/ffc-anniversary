      ...
      <div className="bg-rose-50 py-4">
        <div className="container mx-auto px-4">
          <nav className="flex items-center gap-2 text-sm flex-wrap">
            ...
            <ChevronRight className="h-4 w-4 text-gray-400" />
            <Link href={`/${service.slug}`} className="text-gray-500 hover:text-rose-600">{service.name}</Link>
            <ChevronRight className="h-4 w-4 text-gray-400" />
                    ...
                    <div className="space-y-4">
                      {handcraftedContent!.process.map((step, idx) => (
                        <div key={idx} className="flex gap-4">
                          <div className="flex-shrink-0 w-8 h-8 bg-rose-600 text-white rounded-full flex items-center justify-center font-bold text-sm">
                            {idx + 1}
                          </div>
                          <div>
                ...
                <ul className="space-y-3 mb-8">
                  {(hasUniqueContent ? handcraftedContent!.whyChooseUs : generatedContent.whyChooseUs).map((item, index) => (
                    <li key={index} className="flex items-start gap-3">
                      <Check className="h-5 w-5 text-rose-600 flex-shrink-0 mt-0.5" />
                      <span>{item}</span>
                    </li>
                  ))}
                </ul>
            ...
            {relatedKeywords.map((kw) => (
              ...
              >
                <Card className="border-rose-100 hover:border-rose-300 hover:shadow-md transition-all group">
                  <CardContent className="p-4">
                    <h3 className="font-medium group-hover:text-rose-600 transition-colors flex items-center justify-between">
                      {kw.title}
          ...
          <div className="flex flex-wrap justify-center gap-3 max-w-4xl mx-auto">
            {vadodaraAreas.slice(0, 12).map((area) => (
              <Link 
                key={area.slug}
                href={`/${area.slug}`}
                className="px-4 py-2 bg-rose-50 rounded-full text-gray-700 hover:bg-rose-600 hover:text-white transition-colors border border-rose-200 text-sm"
              >
                {area.name}
              </Link>
            ))}

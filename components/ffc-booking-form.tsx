...
export function FFCBookingFormInner({ pageTitle, variant = 'default', packageName, defaultPackageSlug, onClose }: FFCBookingFormInnerProps) {
  ...
  return (
    <Card className={`${variant === 'modal' ? 'border-0 shadow-none' : 'shadow-lg'} ${variant === 'hero' ? 'bg-white/60 backdrop-blur-md border-white/20' : ''} overflow-hidden`}>
      {variant !== 'modal' && (
        <div className={`${variant === 'hero' ? 'bg-gradient-to-r from-rose-600/95 to-pink-600/95' : 'bg-gradient-to-r from-rose-600 to-pink-600'} p-4 text-white`}>
      ...
      {variant === 'modal' && onClose && (
        <div className="flex justify-between items-center p-4 border-b">
          <div>
            <CardTitle className="text-xl text-rose-600">Reserve Now</CardTitle>
      ...
      <CardContent className="p-6">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    {packages.map((pkg) => (
                      ...
                      >
                        ...
                        <span className="w-4 shrink-0">
                          {selectedPackageSlug === pkg.slug && <Check className="h-3.5 w-3.5 text-rose-600" />}
                        </span>
                        {/* Package info */}
                        ...
                        <button
                          ...
                          className="ml-auto shrink-0 inline-flex items-center gap-1 px-2 py-1 text-[10px] sm:text-xs font-semibold rounded bg-rose-600 text-white hover:bg-rose-700 shadow-sm transition-all hover:shadow-md"
                        >
                          View
          ...
          <Button
            ...
            className="w-full bg-gradient-to-r from-rose-600 to-pink-600 hover:from-rose-700 hover:to-pink-700 text-white py-5 text-base font-semibold mt-2"
          >
            {isSubmitting ? (
              <>
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button className={`bg-gradient-to-r from-rose-600 to-pink-600 hover:from-rose-700 hover:to-pink-700 text-white ${className}`}>
